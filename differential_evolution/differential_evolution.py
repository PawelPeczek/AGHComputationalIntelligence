...
class DifferentialEvolution(EvolutionaryAlgorithm[FloatSolution, FloatSolution]):
    ...
    def reproduction(self, population: List[FloatSolution]) -> List[
        FloatSolution]:
        ...
        for speciman in population:
            ...
            if self.is_simulated_annealing:
                current_radius = self.initial_search_radius * (
                            1 - self.iter / self.max_iter)
                neighbourhood = list(filter(
                    lambda neighbour: 0 < self.calculate_euclidean_distance(neighbour, speciman) <= current_radius, population
                ))
            ...
            if self.is_augmented_reproduction:
                weights = [math.pow(2, -x) for x in
                           range(1, self.number_of_partners + 1)]
            ...
            for index_dim in range(self.dims):
                if random_weights[index_dim] < self.cr or index_dim == dim:
                    if self.is_augmented_reproduction:
                        ...
                        offspring.variables[index_dim] = \
                            partners[0].variables[index_dim] + self.f * \
                            (partners[1].variables[index_dim] - partners[2].variables[index_dim])
            offspring_population.append(offspring)

        return offspring_population

    def replacement(self, population: List[FloatSolution],
                    offspring_population: List[FloatSolution]) -> List[FloatSolution]:
    ...
    def create_initial_solutions(self) -> List[FloatSolution]:
        ...
        return [self.problem.create_solution() for x in
    ...
    def calculate_init_search_radius(self) -> float:
        ...
        for i in range(self.problem.number_of_variables):
            sum_of_squares += (self.problem.upper_bound[i] -
                               self.problem.lower_bound[i]) ** 2
