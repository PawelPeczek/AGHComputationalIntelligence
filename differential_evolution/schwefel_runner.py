...
def run() -> None:
    target_path = os.path.join(RESULTS_DIR, "testDE_Schwefel.json")
    schwefel_problem = Schwefel(number_of_variables=50, lower_bound=-500, upper_bound=500)


    variable_species_size_execution_unit = ExecutionUnit(
        algorithm_cls=DifferentialEvolution,
        problem_name="Schwefel_50dim_variable_species_size"
    ).register_run(
        parameters={
    ...
    variable_number_of_species_execution_unit = ExecutionUnit(
        algorithm_cls=DifferentialEvolution,
        problem_name="Schwefel_50dim_variable_number_of_species"
    ).register_run(
        parameters={
            "problem": schwefel_problem,
            "each_species_size": 10,
    ...
    variable_number_of_partners_execution_unit = ExecutionUnit(
        algorithm_cls=DifferentialEvolution,
        problem_name="Schwefel_50dim_variable_number_of_partners"
    ).register_run(
        parameters={
            "problem": schwefel_problem,
            "each_species_size": 10,
    ...
    variable_iterations_execution_unit = ExecutionUnit(
        algorithm_cls=DifferentialEvolution,
        problem_name="Schwefel_50dim_variable_max_iter"
    ).register_run(
        parameters={
            "problem": schwefel_problem,
            "each_species_size": 20,
