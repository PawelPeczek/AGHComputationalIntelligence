...
class DifferentialEvolutionElitism(EvolutionaryAlgorithm[FloatSolution, FloatSolution]):
    def __init__(self,
                 problem: DEProblem,
                 ...
                 no_partner_for_each_class: List[int] = [3, 5, 7, 9, 11]):
        ...
        self.no_partner_for_each_class = no_partner_for_each_class
        self.no_classes = len(no_partner_for_each_class)
    ...
    def reproduction(self, population: List[FloatSolution]) -> List[FloatSolution]:
        ...
        for species in species_list:
            for index, speciman in enumerate(species):
                neighbourhood = species
                ...
                speciman_class = (index * self.no_classes) // len(neighbourhood)
                no_partners = self.no_partner_for_each_class[speciman_class]

                if len(neighbourhood) < no_partners:
                    offspring_population.append(speciman)
                    continue
                            
                partners = random.sample(neighbourhood, no_partners)
                weights = []

                weights = [math.pow(2, -x) for x in range(1, no_partners + 1)]
